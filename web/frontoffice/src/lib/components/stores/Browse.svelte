...
<div class="relative overflow-x-auto">
    <div class="flex flex-col sm:flex-row md:my-2 mb-6 sm:mb-3">
         <div class="relative">
            ...
            <input bind:value={filter} placeholder="Search store title, description or enter a store id"
                   class="block pl-9 pr-4 py-2 w-96 appearance-none rounded-r rounded-l sm:rounded-l-none border border-gray-400 border-b
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        ...
        <!-- Mobile -->
        <table class="md:hidden table-auto text-sm text-left text-gray-500 dark:text-gray-400 w-fit pr-4">
            <tbody class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
            {#each sortedStalls as [stallId, stall]}
                {#if
                    filter === null ||
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover">
                        <th class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <div>
                                ...
                                {#if stall.createdAt}
                                    <div class="text-xm opacity-50">Since {formatTimestamp(stall.createdAt)}</div>
                                {/if}
                            </div>
                        </th>
                    </tr>
