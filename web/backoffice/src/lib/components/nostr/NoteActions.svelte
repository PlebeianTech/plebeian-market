<script lang="ts">
    ...
    import { NostrPool, NostrPublicKey, Error as ErrorStore } from "$lib/stores";
...
<div class="flex items-center w-full space-x-5 mb-1">
    <!-- LIKE -->
    <button class="btn btn-ghost flex tooltip tooltip-top" data-tip="like" style="color:#6b7280" on:click|preventDefault={() => sendReaction($NostrPool, message.id, message.pubkey, '+')}>
        {#if likeIsMine }
    ...
    {#if message.reactions}
        {#each [...message.reactions] as [reaction, pubkeys]}
            {#if ['+', '-'].indexOf(reaction) === -1}
                <button class="btn btn-ghost" on:click|preventDefault={() => sendReaction($NostrPool, message.id, message.pubkey, reaction)}>
