...
<script lang="ts">
    ...
    $: twitterHref = stallOwner ? `https://twitter.com/${stallOwner.twitterUsername}` : null;
    ...
    {#if stallOwner}
        <div class="w-11/12 md:w-2/3 items-center mx-auto mt-2">
            ...
            <div class="w-full inline-block relative after:pt-[33.33%] after:block after:content-['']">
                <div class="absolute top-0 bottom-0 left-0 right-0 rounded-md bg-center bg-no-repeat bg-cover" style="background-image: url({stallOwner.stallBannerUrl});" alt=""></div>
            </div>

            <div class="flex flex-col-reverse md:flex-row mt-4">
                <div class="mt-4 text-center">
                    <Avatar account={stallOwner} height="24" />
                </div>
                ...
                <div class="md:ml-4 ml-0 bg-base-300 rounded flex-1 p-4">
                    ...
                    <div class="flex float-right">
                        ...
                        <a href={twitterHref} class:link={twitterHref !== null} target="_blank">@{stallOwner.twitterUsername}</a>
                    </div>
                </div>
            </div>
            {#if isMyStall}
