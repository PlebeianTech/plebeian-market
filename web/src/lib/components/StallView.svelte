        ...
        {#if canAddItems || showActiveAuctions || showPastAuctions}
            ...
            {#if canAddItems}
                <ListView
                    ...
                    <div slot="new-entity" class="lg:flex lg:justify-start justify-center" let:setCurrent={setCurrent}>
                        {#if isCampaignStall}
                            <div id="auction-hour-1" class="grid lg:place-items-start w-full p-4" on:click|preventDefault={() => loginAndNewItem(setCurrent, () => new TimeAuction())}>
                              <div class="w-full my-8 grid place-items-center">
                                <!-- <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width={1.5} stroke="currentColor" class="w-24 h-24">
                                  <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg> -->
                                <button class="btn btn-primary font-bold text-center">1 Hour of your time</button>
                              </div>
                            </div>
                        {/if}
            ...
            {#if canAddItems}
                <ListView
                    ...
                    <div slot="new-entity" class="flex justify-start" let:setCurrent={setCurrent}>
                        <div class="my-4 grid w-full p-4" on:click|preventDefault={() => loginAndNewItem(setCurrent, () => new Listing())}>
                          <!-- <div class="w-20 my-8">
