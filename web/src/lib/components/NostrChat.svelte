...
<div>
  ...
  {#if emptyChatShowsLoading && sortedMessages.length === 0}
      <Loading />
  {:else}
      <div class="p-3 bg-black shadow rounded-lg flex items-center space-x-2">
          <textarea
                  rows="2"
                  autofocus
          ...
          <div on:click={sendMessage}
               class="grid place-items-center p-2 border-dark
               hover:bg-neutral-focus transition-all cursor-pointer text-white">
              <svg xmlns="http://www.w3.org/2000/svg" width="54" height="54" fill="currentColor" class="bi bi-telegram" viewBox="-5 -5 24 24">
