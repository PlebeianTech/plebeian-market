    ...
    <div class="chat-image avatar">
        <div class="w-12 rounded-full ring-primary ring-offset-base-100 ring-offset-2" class:ring={!message.samePubKey}>
            {#if !message.samePubKey}
                <img src="{message.profileImage}" alt="profile picture" class:profileInfoImage={!message.samePubKey} />
            {/if}
        </div>
    </div>
    {#if !message.samePubKey}
        <div class="chat-header">
            <span class="mr-3" class:profileInfoName={!message.samePubKey}>{message.profileName}</span>
        </div>
    {/if}
