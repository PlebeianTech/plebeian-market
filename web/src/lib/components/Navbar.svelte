...
<nav class="navbar flex items-center bg-base-300 flex-wrap">
    ...
    <div class="lg:hidden ml-auto pb-2 pt-1 pr-1" on:click={toggleMenu}>
        {#if $token && $user}
    ...
    <div class="{menuDisplay} w-full lg:inline-flex lg:flex-grow lg:w-auto lg:items-center">
        <ul class="lg:inline-flex lg:flex-row lg:ml-auto flex flex flex-col" on:click={closeMenu}>
            {#if $user && !$user.twitterUsernameVerified}
                <li class="lg:inline-flex lg:w-auto pr-3 py-2 rounded btn btn-ghost cursor-pointer">
            ...
            {#if $token && $user}
            <!-- user logged in -->
            <a class="lg:inline-flex lg:w-auto px-3 py-2 rounded btn btn-ghost cursor-pointer" href="{getBaseUrl()}auctions">
                My auctions
            </a>
            <li class="lg:hidden lg:inline-flex lg:w-auto px-3 py-2 rounded btn btn-ghost cursor-pointer" on:click|preventDefault={showProfile}>
                <label class="cursor-pointer" for="profile-modal">Profile</label>
            </li>
            <li class="lg:inline-flex lg:w-auto px-3 py-2 rounded btn btn-ghost cursor-pointer" on:click|preventDefault={showUserNotifications}>
                <label class="cursor-pointer" for="profile-modal">Notifications</label>
            </li>
            {/if}
            <!-- all users -->
            <li class="lg:inline-flex lg:w-auto px-3 py-2 rounded btn btn-ghost cursor-pointer" on:click|preventDefault={toggleTheme}>
                <span>{themeToggle}</span>
            </li>
            <a class="lg:inline-flex lg:w-auto px-3 py-2 rounded btn btn-ghost" href="https://t.me/PlebeianMarket"  target="_blank">
                Telegram
            </a>
            <!-- login / logout -->
            {#if $token && $user}
            <a class="lg:inline-flex lg:w-auto px-3 py-2 rounded btn btn-ghost cursor-pointer" href={getBaseUrl()} on:click|preventDefault={() => { token.set(null); localStorage.removeItem('token');}}>
                Logout
            </a>
            {:else}
            <a class="lg:inline-flex lg:w-auto px-3 py-2 rounded btn btn-ghost cursor-pointer" href="{getBaseUrl()}login">
                Login
            </a>
            {/if}
        </ul>
    </div>
    <div class="hidden lg:block lg:pl-2 ml-auto pb-2 pt-1 pr-2" on:click|preventDefault={showProfile}>
        {#if $token && $user}
        <label for={null} tabindex="0" class:verified={$user.twitterUsernameVerified} class:not-verified={!$user.twitterUsernameVerified} class="btn btn-ghost btn-circle avatar ring-2 ring-primary ring-offset-0">
            <div class="w-10 rounded-full">
                <img src={getProfileImage($user)} alt="Avatar" />
